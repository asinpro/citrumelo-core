...
function removeSceneManagerSceneDataFromArray(
    st: SceneManagerSceneData,
    v: SceneManagerSceneData[]
) {
    let i = v.indexOf(st)
    if (i > -1) {
        v.splice(i, 1)
...
export class SceneManager {
    ...
    public setCurrentScene(value: IScene): void {
        ...
        this._lastCreatedScene = new SceneManagerSceneData(
            'scene(anonymous)',
            value
        )
    ...
    private collectScenes() {
        if (this._scenesToDestroy.length > 0) {
            this._scenesToDestroy.forEach((sceneData) => {
                this.removeSceneFromEngine(sceneData)
                removeSceneManagerSceneDataFromArray(
                    sceneData,
                    this._runningScenes
                )
            })
